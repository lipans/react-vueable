...
type TickReturnType<T extends ((...args: any[]) => any) | undefined> = T extends (...args: any[]) => any ? ReturnType<T> extends Promise<any> ? ReturnType<T> : Promise<ReturnType<T>> : Promise<void>;

export default function useNextTick(): () => Promise<void>;
export default function useNextTick<T extends (...args: any[]) => any>(tick: T): (...args: Parameters<T>) => TickReturnType<T>;
export default function useNextTick<T extends ((...args: any[]) => any) | undefined>(tick?: T) {
  const tickParamRef = useRef<{ resolve: (value: T extends (...args: any[]) => any ? ReturnType<T> : void) => void; reject: (reason?: any) => void; args: T extends (...args: any[]) => any ? Parameters<T> : [] }>();
